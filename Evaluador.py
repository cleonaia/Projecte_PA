class Evaluador:
    ...
    @staticmethod
    def calcular_mae(prediccions: List[float], valoracions_reals: List[float]) -> float:
    ...
    @staticmethod
    def calcular_rmse(prediccions: List[float], valoracions_reals: List[float]) -> float:
        if not prediccions or len(prediccions) != len(valoracions_reals):
            return 0
    ...
    @staticmethod
    def evaluar_recomendador(recomanador, dataset: DatasetBase, usuari_id: str) -> Tuple[float, float]:
        ...
        for item_id, puntuacio_real in valoracions_usuari.items():
            # Llamamos a la función de predicción del recomendador
        ...
        mae = Evaluador.calcular_mae(prediccions, valoracions_reals)
        rmse = Evaluador.calcular_rmse(prediccions, valoracions_reals)
